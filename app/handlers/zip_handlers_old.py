...
async def import_zip_archive(message: Message, session: AsyncSession = get_session()):
    ...
    try:
        ...
        if not proj:
            await message.answer("Сначала выберите проект: /project <name>")
            return
        ...
        await message.answer("📦 Обрабатываю ZIP архив...")
        ...
        if not text_files:
            await message.answer("⚠️ В архиве не найдено обрабатываемых текстовых файлов")
            return
        ...
        await message.answer(
            f"✅ ZIP архив импортирован в проект <b>{proj.name}</b>\\n\\n"
            f"📁 Архив: {doc.file_name}\\n"
            f"📊 Файлов обработано: {processed_count} из {stats['text_files']} текстовых\\n"
            f"🏷️ Теги: {tags_str}\\n"
            f"🔗 URI архива: {zip_uri or '—'}\\n\\n"
            f"📈 Статистика:\\n"
            f"  • Всего файлов: {stats['total_files']}\\n"
            f"  • Текстовых: {stats['text_files']}\\n"
            f"  • Бинарных: {stats['binary_files']}"
        )
        
    except Exception as e:
        logger.error(f"Error importing ZIP: {e}")
        await message.answer(f"❌ Ошибка при импорте ZIP: {str(e)}")
...
async def generate_single_file_handler(message: Message, session: AsyncSession = get_session()):
    ...
    try:
        ...
        if not message.text:
            await message.answer("❌ Текст команды не найден")
            return
            
        if "path=" not in message.text or '"' not in message.text:
            await message.answer(
                "Формат команды:\\n"
                "/genfile path=app/module/file.py \\"описание правки\\"\\n\\n"
                "Пример:\\n"
                "/genfile path=src/auth/login.py \\"Добавить двухфакторную аутентификацию\\""
            )
            return
        ...
        if len(desc_parts) < 2:
            await message.answer("Укажите описание в кавычках")
            return
            
        task_description = desc_parts[1].strip()
        
        await message.answer(f"🤖 Генерирую файл {file_path}...")
        ...
        caption = (
            f"📝 Сгенерирован файл для проекта <b>{proj.name}</b>\\n\\n"
            f"📁 Путь: {file_path}\\n"
        ...
        await message.answer(f"❌ Ошибка при генерации файла: {str(e)}")
...
async def diff_zip_archives(message: Message, session: AsyncSession = get_session()):
    ...
    try:
        ...
        if not message.reply_to_message or not message.reply_to_message.document:
            await message.answer("Нужно ответить командой /diffzip на сообщение с ZIP файлом")
            return
        ...
        await message.answer("🔍 Сравниваю архивы...")
        ...
        if not latest_zip_artifact or not latest_zip_artifact.uri:
            await message.answer("❌ Не найден предыдущий архив для сравнения")
            return
        ...
        if not old_zip_data:
            await message.answer("❌ Не удалось загрузить предыдущий архив")
            return
        ...
        response = f"📊 <b>Сравнение архивов</b>\\n\\n{summary}\\n\\n{analysis}\\n\\n🔗 Полный diff: {diff_uri}"
        
        # Split long messages
        if len(response) > 4000:
            await message.answer(f"📊 <b>Сравнение архивов</b>\\n\\n{summary}")
            await message.answer(analysis)
            await message.answer(f"🔗 Полный diff: {diff_uri}")
        else:
        ...
        if len(full_diff.encode('utf-8')) < 10 * 1024 * 1024:  # 10MB limit
            ...
            await message.answer_document(
                diff_doc,
                caption=f"📋 Полный diff между архивами"
            )
        
    except Exception as e:
        logger.error(f"Error comparing ZIP archives: {e}")
        await message.answer(f"❌ Ошибка при сравнении архивов: {str(e)}")