            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    for rel_path, content in iter_text_files(tmp_path, spec):
                        title = f"{stt.last_doc_name or 'archive'}:{rel_path}"
                        await create_import(
                            st, proj,
                            title=title,
                            text=content,
                    ...
                    # Get chat_on flag to rebuild keyboard with correct state
                    chat_on, *_ = await get_chat_flags(st, cb.from_user.id if cb.from_user else 0)
                    await cb.message.answer(f"Импортировано из ZIP в проект: <b>{escape(proj.name)}</b>\nФайлов: {imported_count}\nФайл: {escape(stt.last_doc_name or 'archive')}", reply_markup=build_reply_kb(chat_on))
                    return await cb.answer()
...
async def ask_templates(cb: CallbackQuery):
    ...
    if cb.message and isinstance(cb.message, Message):
        ...
        async with session_scope() as st:
            ...
            await cb.message.answer("✅ Ответ сгенерирован", reply_markup=build_reply_kb(chat_on))
    await cb.answer()
...
async def projects_new(cb: CallbackQuery):
    ...
    if cb.message and isinstance(cb.message, Message):
        ...
        async with session_scope() as st:
            chat_on, *_ = await get_chat_flags(st, cb.from_user.id if cb.from_user else 0)
            await cb.message.answer("Название нового проекта:", reply_markup=ForceReply(selective=True, chat_on=chat_on))
    await cb.answer()
